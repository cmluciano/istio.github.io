...
1. _**Deployments with app label**:_ It is recommended that Pods deployed using
  ...
  Deployment anspecification. Each deployment specification should have a
...
The `istio-inject` ConfigMap in `istio-system` namespace the default 
...
define injected containers/volumes uses this data. 
